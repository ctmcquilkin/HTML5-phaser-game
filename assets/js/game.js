...
var Game = {
	...
	preload: function() {
		...
		// In this case, that's just two squares - one for the snake body and one for the apple.
	...
	create: function() {
		...
		// global variables in the create function initialize on game start
		...
		score = 0;		      // Game score.
		speed = 0;			  // Game speed.
		updateDelay = 0; 	  // A variable to control the update rate
		...
		// Beginning at x = 150, y = 150 and incrementing the x value on every iteration.
		for (var i = 0; i < 10; i++) {
			snake[i] = game.add.sprite(150 + i * squareSize, 150, 'snake'); // Parameters: (x coordinate, y coordinate, image)
		}
	...
	update: function() {
		// the update function is called constantly at a high rate (about 60fps),
		...
		// Since the update function of Phaser has an update rate of about 60fps, 
		...
		if (updateDelay % (10 - speed) == 0) {
			...
			// If a new direction has been chosen from the keyboard, make it the direction of the snake
			if (new_direction) {
			...
			// Increase the length of the snake if an apple has been eaten.
	...
	wallCollision: function(head) {
		...
		if (head.x >= 600 || head.x < 0 || head.y >= 450 || head.y < 0) {
			...
			// If it's not in bounds, we've hit a wall. Show game over screen.
			game.state.start('Game_Over');
		}
	},
